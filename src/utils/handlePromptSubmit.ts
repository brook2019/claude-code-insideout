...
export async function handlePromptSubmit(
  ...
  logger.trace('PROMPT', `handlePromptSubmit called with input: ${params.input ?? 'undefined'}, mode: ${params.mode ?? 'undefined'}`)
  ...
  logger.trace('PROMPT', `finalInput after expansion: ${finalInput}`)
...
async function executeUserInput(params: ExecuteUserInputParams): Promise<void> {
  logger.trace('PROMPT', `executeUserInput`)
  const {
    messages,
    mainLoopModel,
    ideSelection,
  ...
  try {
    ...
    await runWithWorkload(turnWorkload, async () => {
      for (let i = 0; i < commands.length; i++) {
        ...
        const isFirst = i === 0
        logger.trace('PROMPT', `executeUserInput processing command ${i + 1}/${commands.length}: value="${cmd.value}", mode="${cmd.mode}", isFirst=${isFirst}`)
        ...
        logger.trace('PROMPT', `processUserInput result: shouldQuery=${result.shouldQuery}, messages.length=${result.messages.length}, model=${result.model ?? 'default'}`)
        if (isFirst) {
      ...
      if (newMessages.length) {
        ...
        logger.trace('PROMPT', `Calling onQuery - shouldQuery=${shouldQuery}, allowedTools=${JSON.stringify(allowedTools ?? [])}, model=${model ?? mainLoopModel}, primaryInput="${primaryInput ?? 'none'}", newMessages.length=${newMessages.length}`)
        await onQuery(
          newMessages,
          abortController,
          shouldQuery,
